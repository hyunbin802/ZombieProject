...
public class StageManager : MonoBehaviour
{
    ...
    {
        SendChatMessage(chatInputField != null ? chatInputField.text : string.Empty);
    }

    // InputField OnEndEdit 이벤트에 연결
    public void OnEndEditChat(string inputMsg)
    {
    ...
    void SendChatMessage(string inputMsg)
    {
        ...
        string safeMsg = EscapeRichText(inputMsg.Trim());
        string msg = "\n[" + PhotonNetwork.player.NickName + "] " + safeMsg;

        // 채팅은 접속/종료 로그와 달리 과거 이력을 계속 누적하지 않도록 비버퍼 RPC로 전달
