...
public class BaseCtrl : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (!isEnemyDetected)
        {
            head.RotateAround(targetTr.position, Vector3.up, Time.deltaTime * 55.0f);
    ...
    */
        ...
        while (!isDie)
        {
            ...
            foreach (GameObject _Enemy in Enemys)
            {
                if ((_Enemy.transform.position - head.position).sqrMagnitude < dist)
                {
                    ...
                    dist = (EnemyTargets.position - head.position).sqrMagnitude;
                }
            }
    ...
    IEnumerator ShotSetting()
    {
        while (!isDie)
        {
            ...
            dist2 = Vector3.Distance(head.position, EnemyTarget.position);
